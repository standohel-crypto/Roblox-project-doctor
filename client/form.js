...
function renderIssues() {
  ...
  for (const [index,issue] of (result?.issues || []).entries()) {
    ...
    if (issue.originalMessage) { const details=element('details'); details.append(element('summary',t('original')),element('p',issue.originalMessage)); card.append(details); }
...
$('userForm').addEventListener('submit',async event=>{
  ...
  async function runAI(){
    ...
    }catch{aiResult={status:'unavailable',reason:'connection'};}
    finally{renderAI();}
  }
...
function renderAI(){
  ...
  if(aiResult.status!=='done'){
    const reasons={invalid_response:['ИИ вернул ответ в неверном формате. Попробуй ещё раз.','AI returned an invalid format. Try again.'],truncated:['Ответ ИИ не поместился целиком. Попробуй проверить меньший фрагмент кода.','The AI response was incomplete. Try a smaller code sample.'],rate_limit:['Лимит Groq временно исчерпан. Попробуй позже.','Groq rate limit reached. Try later.'],not_configured:['На сервере не настроен ключ Groq.','Groq key is not configured on the server.'],configuration:['Проверь ключ и доступ к модели на сервере.','Check the server API key and model access.'],timeout:['ИИ не успел ответить. Попробуй позже.','AI review timed out. Try later.']};
    const msg=reasons[aiResult.reason]||['ИИ временно недоступен.','AI is temporarily unavailable.'];
    panel.append(element('p',msg[ru?0:1]),element('p',ru?'Результат проверки Luau показывается отдельно.':'Luau results are shown separately.'));return;
  }
...
(() => {
    ...
    function renderVisitorStats() {
        ...
        statsElement.textContent = ru
            ? `Онлайн: ${online} · Сегодня: ${today}`
            : `Online: ${online} · Today: ${today}`;

        statsElement.title = ru
            ? 'Уникальные браузеры. День по времени Казахстана.'
            : 'Unique browsers. Day measured in Kazakhstan time.';
    }

...
// Never log the provider's raw body: it may contain submitted code.
export function providerFailure(status, payload) {
 const error=payload?.error;
 const code=typeof error?.code==='string'?error.code:'';
 const message=typeof error?.message==='string'?error.message:'';
 if(status===413 || /context_length|context_window|request_too_large/.test(code) || /context (?:length|window)|reduce the length|request too large|tokens per minute.*requested/i.test(message))return 'context_limit';
 if(status===429)return 'rate_limit';
 if([401,403,404].includes(status)||/model_not_found|model_decommissioned/.test(code))return 'configuration';
 if(status===400 && /json_validate_failed|json_schema|structured_output/.test(code+' '+message))return 'invalid_response';
 if(status===400)return 'provider_request';
 return 'provider_error';
}
function unavailable(reason, startedAt, httpStatus) {
...
export async function reviewCode(code, language='ru') {
 ...
 try {
  ...
  if(!response.ok) {
   ...
   return fail(providerFailure(response.status,payload),response.status);
  }
  const data=await response.json();const choice=data.choices?.[0];const content=choice?.message?.content;
  if(typeof content!=='string'||!content.trim())return fail('empty_response');
  if(choice.finish_reason==='length')return fail('truncated');
  let review;try{review=parseReview(content,code);}catch{return fail('invalid_response');}
  return {status:'done',...review,language};
 }catch(error){return fail(['TimeoutError','AbortError'].includes(error.name)?'timeout':'connection');}
}
